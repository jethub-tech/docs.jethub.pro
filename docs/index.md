...
- Аргументы
    ...
    | `--sast-python-skiptest`     | Список идентификаторов уязвимостей (`id`) для игнорирования   | Через запятую, без пробела | Только для САК-python. В строке уязвимости можно использовать `# nosec` или `// nosec`  |
